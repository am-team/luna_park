...
module LunaPark
  module Repositories
    ...
    #     end
    #
    #     # same as previous method `create`, but using helper
    #     def save(wrappable)
    #       modify_entity(wrappable) { |entity, row| dataset.returning.where(id: entity.id).update(row) }
    #     end
    #
    #     def dataset
    #       SEQUEL_DB[:users]
    ...
    class Sequel
      class << self
        ...
        def modify_entity(input)
          entity = wrap(input)
          row = to_row(entity)

          new_row = yield(entity, row)

          new_attrs = from_row(new_row)
          entity.set_attributes_from_source(new_attrs)
          entity
        end
